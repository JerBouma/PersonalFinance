...
from personal_finance import cashflow_model, excel_model
...
class Cashflow:
    ...
    def __init__(
        ...
        self._description_columns: list[str] | None = self._cfg["general"]["description_columns"]
        self._amount_column: str | None = self._cfg["general"]["amount_columns"]
        self._cost_or_income_column: str | None = self._cfg["general"]["cost_or_income_columns"]
    ...
    def read_cashflow_dataset(
        ...
        """
        excel_location = excel_location if excel_location else self._cfg["general"]["file_location"]
        adjust_duplicates = adjust_duplicates if adjust_duplicates else self._cfg["general"]["adjust_duplicates"]
        date_column = date_column if date_column else self._cfg["general"]["date_columns"]
        date_format = date_format if date_format else self._cfg["general"]["date_format"]
        description_columns = description_columns if description_columns else self._cfg["general"]["description_columns"]
        amount_column = amount_column if amount_column else self._cfg["general"]["amount_columns"]
        cost_or_income_dict = cost_or_income_dict if cost_or_income_dict else self._cfg["general"]["cost_or_income_columns"]
        decimal_seperator = decimal_seperator if decimal_seperator else self._cfg["general"]["decimal_separator"]
        ...
        if self._cash_flow_dataset.empty:
            ...
            self._cost_or_income_criteria) = cashflow_model.read_cashflow_dataset(
                excel_location=excel_location,
    ...
    def apply_categorization(
        ...
        description_columns = description_columns if description_columns else self._description_columns
        categorization_threshold = categorization_threshold if categorization_threshold else self._cfg["general"]["categorization_threshold"]
        direct_match = direct_match if direct_match else self._cfg["general"]["direct_match"]
        ...
        categorized_percentage = (len(self._cash_flow_dataset[self._cash_flow_dataset['category'] != 'Other']) / len(self._cash_flow_dataset['category'])) * 100
        
        self._highest_match_percentage = pd.Series(total_matches)
        
        print(f"{categorized_percentage:.2f}% of the cash flow dataset has been categorized.")
        
        if (self._highest_match_percentage < categorization_threshold).any():
            print("The following keywords have not led to any category matches. Please consider removing or updating these keyword to keep things compact:")
            below_threshold = self._highest_match_percentage[self._highest_match_percentage < categorization_threshold].sort_values()
    ...
    def apply_cost_or_income_indicator(
        ...
        """
        cost_or_income_column = cost_or_income_column if cost_or_income_column else self._cost_or_income_column
        cost_or_income_criteria = cost_or_income_criteria if cost_or_income_criteria else self._cost_or_income_criteria
                
        amount_column = amount_column if amount_column else self._amount_column

        if not cost_or_income_column:
            print("No cost or income indicator columns found in the cash flow dataset. Please specify the columns in the configuration file.")
        elif not cost_or_income_criteria:
            print("No amount columns found in the cash flow dataset. Please specify the columns in the configuration file.")
        else:
            for indicator, multiplier in cost_or_income_criteria.items():
                self._cash_flow_dataset.loc[self._cash_flow_dataset[cost_or_income_column] == indicator, amount_column] *= multiplier
    ...
    def group_transactions_by_periods(self) -> None:
        ...
        """
        if self._cash_flow_dataset.empty:
            raise ValueError("No cash flow dataset found. Please run the 'read_cashflow_dataset' function first.")
    
        self._yearly_cash_flow_dataset = self._cash_flow_dataset.groupby(pd.Grouper(freq="Y")).apply(lambda x: x)
        self._quarterly_cash_flow_dataset = self._cash_flow_dataset.groupby(pd.Grouper(freq="Q")).apply(lambda x: x)
        self._monthly_cash_flow_dataset = self._cash_flow_dataset.groupby(pd.Grouper(freq="M")).apply(lambda x: x)
    ...
    def create_period_overviews(self, period: str) -> pd.DataFrame:
        ...
        Raises:
            ...
            ValueError: If an unsupported 'period' value is provided. Supported values are 'year', 'quarter',
                or 'month'.
        """
        if self._yearly_cash_flow_dataset.empty and self._quarterly_cash_flow_dataset.empty and self._monthly_cash_flow_dataset.empty:
            raise ValueError("No period cash flow datasets found. Please run the 'group_transactions_by_periods' function first.")     
        ...
        if period == "year":
            ...
            raise ValueError("Period not supported. Please use 'year', 'quarter' or 'month'.")
        ...
        for period_value in period_values:
            period_data = period_dataset.loc[period_value].groupby('category').agg({self._amount_column: 'sum'})
            period_data = period_data.reindex(categories).fillna(0)

            period_cash_flows.loc[:, period_value] = period_data.values
    ...
    def create_excel_template(self, excel_file_name: str | None):
        ...
        excel_file_name = excel_file_name if excel_file_name else self._cfg["excel"]["excel_file_name"]
        ...
        if not self._quarterly_overview.empty:
            excel_model.create_overview_excel_report(
                writer, dataset=self._quarterly_overview, sheet_name="Quarterly Overview")
