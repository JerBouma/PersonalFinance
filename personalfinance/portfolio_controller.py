...
from personalfinance import portfolio_model, excel_model, helpers
...
class Portfolio:
    ...
    def __init__(
        ...
        # self._name_column: list[str] | None = self._cfg["general"][
        #     "name_columns"
        # ]
        # self._ticker_columns: str | None = self._cfg["general"]["ti9ck"]
    ...
    def read_portfolio_dataset(
        ...
        name_columns = (
            name_columns
            if name_columns
            else self._cfg["general"]["name_columns"]
        )
        ticker_columns = (
        ...
        price_columns = (
            price_columns
            if price_columns
            else self._cfg["general"]["price_columns"]
        )
        
        volume_columns = (
            volume_columns
            if volume_columns
            else self._cfg["general"]["volume_columns"]
        )
        
        costs_columns = (
            costs_columns
            if costs_columns
            else self._cfg["general"]["costs_columns"]
        )
